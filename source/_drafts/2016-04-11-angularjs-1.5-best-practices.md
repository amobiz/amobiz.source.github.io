...
* 雖然 AngularJS 2.0 發佈在即 (據說 5 月正式發佈)，但是既有的 AngularJS 1.x 程式碼仍需要維護。
...
本文主要基於 [Sane, scalable Angular apps are tricky, but not impossible. Lessons learned from PayPal Checkout.](https://medium.com/@bluepnume/sane-scalable-angular-apps-are-tricky-but-not-impossible-lessons-learned-from-paypal-checkout-c5320558d4ef)
 以及 [Exploring the Angular 1.5 .component() method](https://toddmotto.com/exploring-the-angular-1-5-component-method/) 兩篇文章。
...
* 必然是 isolated scope，也就是不會繼承 parent scope。因此，若需要外部 scope 的資料，必須明確在 `scope` 或 `bindings` 中定義屬性，並在使用 component 時傳入。
* 如果你沒有自行指定 `controllerAs`，則會自動幫你指定為 `$ctrl`，也就是說，強迫必須使用 `controllerAs` 語法。

#### Scope 有三種

1. isolated scope: `scope: {}`

	強烈建議一律使用 "isolated scope"，不要使用另外兩種。當需要外部 scope 資料時，請明確定義屬性，並由外部指定傳入。
...
只要遵循上面永遠使用 "isolated scope" 的原則，或者是了解什麼是 `controller as`，知道 AngularJS 過去最被詬病的 scope 黑暗歷史，可以跳過這一段。
...
這一篇說明為何 1.3 需要引進 `bindTocontroller'，以及 1.4 的演進：
...
然後，1.5 引進了上面介紹過的 `.component()` 函數，其中又引入新的 `bindings`，可以完全取代 `bindTocontroller'。
