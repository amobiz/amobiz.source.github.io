...
1. isolated scope: `scope: {}`
	...
	建立一個與世隔絕的 scope。強烈建議一律使用 "isolated scope"，不要使用另外兩種。當需要外部 scope 資料時，請明確定義屬性，並由外部指定傳入。
...
然後，1.5 引進了上面介紹過的 `.component()` 函數，其中又引入新的 `bindings`，用來完全取代 `scope` 及 `bindTocontroller'。
