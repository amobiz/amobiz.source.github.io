...
我過去一直以為 factory 函數每次被要求注入的時候都會執行一次，但實際上卻沒有。也就是說，不論是 factory, service 或 provider，實際上建立的都是 singleton service，建立之後的行為完全一樣，唯一的差別就是建立的形式不同。AngularJS team 真的是取了很不好的名稱，並且把一件簡單的事情極度複雜化了。

至於實際上要建立服務時，我們透過 `factory()`, `service()` 或 `provider()` 函數來建立：

```js
angular.module('app', [])
.provider('myProvider', function PactoryConstructor() {
