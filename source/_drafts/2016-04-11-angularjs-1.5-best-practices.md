...
### 就使用 Factory (忘記 Service 和 Provider 吧)
...
2. 它們全都是 singleton，都只會被執行一次，其執行產出結果就是即將被拿來做為注入使用的物件，
3. provider 的存在，只是為了提更改預設值的初始化功能，但其實有許多方式可以做到，並不是非 provider 不可，詳細說明請參考下一條建議，
4. factory 比 service 有彈性。

不過這邊還是透過模擬它們的實作方式，稍微說明一下它們的差異。簡單地說，它們的差別可以用下面這段虛擬碼表達，為了簡單起見，dependency injection 處理的部份完全不列入。
...
function service(serviceName, ServiceConstructor) {
	return provider(serviceName, function () {
		this.$get = function () {
...
1. 以 `new` 運算子呼叫我們提供的 `ProviderConstructor()` 建構子，建立一個物件，這個物件稱為 provider，
2. 將此 provider 物件以 `serviceName + 'Provider'` 的名稱，儲存為一個服務，這個服務將只能透過 '.config()` 函數使用，
3. 呼叫此物件的 `.$get()` 方法，取得將被用來注入的物件，這個物件才是真正的 service 物件，
4. 將 service 物件以我們提供的 `serviceName` 儲存為一個服務，這個服務將能夠在 '.run()`, `factory()`, `service()`, `provider()`, `controller()`, `directive()` ... 等函數，以及任意建構子中使用。
5. 上面的過程，在整個應用程式生命週期中，只會執行一次。

而 `service()` 函數，則是:

1. 轉呼叫上面的 `provider()` 函數，
2. 傳入的函數參數將被當作是一個建構子，將以 `new` 運算子呼叫，建立一個具有 `$get()` 方法的物件，
3. 該物件將以 `serviceName + 'Provider'` 的名稱，儲存為一個服務，
4. 該物件的 `$get()` 方法被呼叫時，將以 `new` 運算子呼叫我們提供的 `ServiceConstructor()` 建構子，並將建立的物件回傳，這個物件才是真正的 service 物件，
5. 該 service 物件將以我們提供的 `serviceName` 儲存為一個服務。
6. 上面的過程，在整個應用程式生命週期中，只會執行一次。
...
2. 傳入的函數參數將被當作是一個建構子，將以 `new` 運算子呼叫，建立一個具有 `$get()` 方法的物件，
3. 該物件將以 `serviceName + 'Provider'` 的名稱，儲存為一個服務，
4. 該物件的 `$get()` 方法被呼叫時，將直接呼叫我們提供的 `factoryFunction()` 函數，`factoryFunction()` 函數回傳的任意值，都會被當作 service 物件，
5. 該 service 物件將以我們提供的 `serviceName` 儲存為一個服務。
6. 上面的過程，在整個應用程式生命週期中，只會執行一次。
...
1. 每個服務，實際上都會伴隨著一個 provider 服務，但只有透過 `provider()` 函數，才有機會附加其他額外方法，而 `factory()` 和 `service()` 都只會有 `$get()` 方法，
2. 它們全部都只有在註冊的時候，被執行唯一的一次，其結果就被儲存下來成為 singleton，再也沒有機會改變。
3. 不管在哪個模組註冊，註冊的服務都是全域共用的。

我過去一直以為 factory 函數與 service 的差異是，它每次被要求注入的時候都會執行一次，但實際上卻沒有，它跟 service 一樣，總是只被呼叫一次。也就是說，不論是 factory, service 或 provider，實際上建立的都是 singleton service，建立之後的行為完全一樣，__它們之間真正的差別就是建立的形式不同__。AngularJS team 真的是取了很不好的名稱，並且把一件簡單的事情極度複雜化了。

由下面的範例就可以看出來：

1. 雖然建立的是 `personFactory`，但是卻同時存在有 `personFactoryProvider`，
2. `personFactory.rtti` 的值，在兩個 controller 中相同，表示 `personFactory` 的確是同一個物件，
3. 由 `PersonController1` 透過 `personFactory.defaults()` 函數設定的值，在 `PersonController2` 可以看得到。
...
至於實際上要建立服務時，我們透過 `factory()`, `service()` 或 `provider()` 函數來建立：
...
.service('myService', function ServiceConstructor() {
	function MyServiceConstructor() {
	}
	MyServiceConstructor.prototype.method = function () {
	};
	return MyServiceConstructor;
}
...
可以看到，雖然 provider 的彈性最大，也能提供設定功能，但是如下一段建議所言，設定功能有很多方式可以達成，因此並沒有非使用 provider 不可的理由。而 service，由於強制必須提供 constructor，所以服務本身只能是物件類型，彈性上多少受到一些限制。基於這些理由，多數的人才會建議：一律使用 factory。
...
老實說，我比較不建議這個作法。透過 DI 應該直接取得立即可用的物件或服務，應該依賴於其界面，而不是其實作。
